...
Для запуска линтеров после внесения изменений в код перед каждым коммитом 
реализован pre-commit хук, который автоматически после каждой команды <span style="color: green;">git commit -m "any_text"</span> 
запустит проверку кода соответствующими линтерами       
...
pytest
```
В проекте реализовано измерения покрытия кода тестами, цель которого не гнаться за 100%, 
а использовать эту информацию для выявления слабых мест в тестовом наборе 
и осознанного улучшения процесса тестирования. Измерение покрытия осуществлено с помощью плагина pytest-cov
который интегрирован с coverage.py (основной библиотекой для измерения покрытия в Python).
Для более наглядного и детального анализа покрытия использованы HTML-отчеты, которые 
позволяют увидеть прямо в браузере, какие строки кода были покрыты, а какие нет.
Запустите в терминале команду
```bash
pytest --cov=PythonProject2 --cov-report=html tests/
```
Эта команда создаст в проекте директорию htmlcov (по умолчанию). 
Откройте файл htmlcov/index.html (нажав на него ПКМ-> Open in-> Browser-> выберите браузер), 
чтобы увидеть интерактивный отчет.
