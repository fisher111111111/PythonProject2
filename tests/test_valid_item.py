...
from PythonProject2.src.utils_item.validator_item_data import (
...
class TestValid:
    ...
    def test_check_all_items(self, valid_scenarios):
        ...
        assert (
            len(json_items.get("data")) and json_items.get("count") != 0
        ), "Полученный объект пуст"
    ...
    def test_create_check_in_all_items(self, valid_scenarios, cleanup_items):
        ...
        assert (
            json_item_create in json_items["data"]
        ), "созданного item нет в общем списке"
    ...
    def test_create_multiple_items(self, valid_scenarios, cleanup_items):
        ...
        assert (
            len(items_responses) == multiple
        ), f"Создано {len(items_responses)} элементов вместо ожидаемого количества"
        json_items = [obj.json() for obj in items_responses]
        items_ids = [item.get("id") for item in json_items]
        assert len(set(items_ids)) == len(
            items_ids
        ), "Некоторые созданные элементы имеют одинаковые ID"
        for response_obj in items_responses:
            validate_item(
                response_obj, model=ResponseItem, expected_data=response_obj.json()
            )
        for item_id in items_ids:
            cleanup_items.append(item_id)
        assert all(
            response.status_code == 200 for response in items_responses
        ), "Получен неожидаемый статус-код"
        assert all(
            len(json_item) > 0 for json_item in json_items
        ), "JSON не должен быть пустым"
        assert all(
            json_item.get("title") not in (None, "") for json_item in json_items
        ), "Значение ключа 'title' пустое"
        assert all(
            item_id not in (None, "") for item_id in items_ids
        ), "Значение ключа 'id' пустое"
        assert all(
            json_item.get("owner_id") not in (None, "") for json_item in json_items
        ), "Значение ключа 'owner_id' пустое"
    ...
    def test_filter_items(self, valid_scenarios):
        ...
        validate_items(
            items_responses, model=ResponseAllItems, expected_data=json_items
        )
        assert (
            len(data) <= expected_count
        ), f"Получено {len(data)} элементов вместо ожидаемого количества"
