...
class AuthLogin:
    ...
    def create_token(self):
        ...
        response_token = self.auth_session.post(
            self.login, headers=self.headers, data=auth_data
        )
    ...
    def auth_wrong_data(self):
        ...
        response_token = self.auth_session.post(
            self.login, headers=self.headers, data=auth_wrong
        )
        return response_token

    def non_auth_data(self):
        """Отправляет запрос на создание токена аутентификации."""
        none_auth = {"username": os.getenv("None"), "password": os.getenv("None")}
        response_token = self.auth_session.post(
            self.login, headers=self.headers, data=none_auth
        )
        return response_token

    def empty_auth_data(self):
        """Отправляет запрос на создание токена аутентификации."""
        empty_auth = {"username": "", "password": ""}
        response_token = self.auth_session.post(
            self.login, headers=self.headers, data=empty_auth
        )
        return response_token
