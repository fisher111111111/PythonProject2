...
from PythonProject2.src.enums_item.const_url import ApiHeaders, ConstURL
...
class ItemsApi:
    ...
    def create_item(self):
        ...
        allure.attach(
            str(response),
            name="Результат запроса на создание item",
    ...
    def get_all_items(self, params=None):
        ...
        response = self.session.get(
            f"{self.base_url}", params=params, headers=token_item.headers
        )
        allure.attach(
            str(response),
            name="Результат запроса на получение всех items",
            attachment_type=allure.attachment_type.TEXT,
        )
        if response.status_code != 200:
            response.raise_for_status()
    ...
    def get_item(self, item_id):
        token_item = self.token
        response = self.session.get(
            f"{self.base_url}{item_id}", headers=token_item.headers
        )
        allure.attach(
            str(response),
            name="Результат запроса на получение item",
            attachment_type=allure.attachment_type.TEXT,
        )
        if response.status_code != 200:
            response.raise_for_status()
    ...
    def update_item(self, item_id):
        ...
        response = self.session.put(
            f"{self.base_url}{item_id}", headers=token_item.headers, json=item_update
        )
        allure.attach(
            str(response),
            name="Результат запроса на обновление item",
            attachment_type=allure.attachment_type.TEXT,
        )
        if response.status_code != 200:
            response.raise_for_status()
    ...
    def delete_item(self, item_id):
        ...
        allure.attach(
            str(response),
            name="Результат запроса на удаление item",
            attachment_type=allure.attachment_type.TEXT,
        )
        if response.status_code != 200:
            response.raise_for_status()
