...
from PythonProject2.src.enums_item.const_url import AuthHeaders, ConstURL
...
class AuthData:
    ...
    def empty_token(self):
        ...
        response.raise_for_status()
        token_session.headers.update({"Authorization": ""})
        return token_session


"""Класс моделей создаваемых данных"""
...
class RequestItem(BaseModel):
    ...
    def item_data(cls) -> "RequestItem":
        ...
        object_item = cls(
            ...
            description=(
                fake.text(max_nb_chars=random.randint(5, 10))
    ...
    def update_item_data(cls) -> "RequestItem":
        ...
        object_item = cls(
            ...
            description=(
                fake.text(max_nb_chars=random.randint(15, 20))
                if random.choice([True, False])
                else None
            ),
        )
        if (
            object_item.description is None
        ):  # Если description равен None, меняем его на подходящий формат
            object_item.description = ""
        allure.attach(
            str(object_item),
            name="Данные для обновления item",
            attachment_type=allure.attachment_type.JSON,
        )
        return object_item.model_dump()
    ...
    def params_valid(
        cls,
    ):  # не стал делать невалидные данные для params, т.к. они работают некорректно!!!
        """Генерация недефолтных параметров - проверка работы выборки сервера"""
        skip = 0
        allure.attach(
            str(skip), name="Значение SKIP", attachment_type=allure.attachment_type.JSON
        )
...
class WrongRequestItems(BaseModel):
    ...
    def too_long_data(cls) -> "WrongRequestItems":
        ...
        object_item = cls(title=result, description=fake.text(max_nb_chars=15))
        allure.attach(
            str(object_item),
            name="Данные для title > 255 символов",
            attachment_type=allure.attachment_type.JSON,
        )
        return object_item.model_dump()
...
class NoneItems(BaseModel):
    ...
    def none_item_data(cls) -> "NoneItems":
        object_item = cls(title="", description=fake.text(max_nb_chars=15))
        allure.attach(
            str(object_item),
            name="Данные для item = None",
            attachment_type=allure.attachment_type.JSON,
        )
        return object_item.model_dump()
