...
class BadScenarioCreate:
    ...
    def create_too_long_title(self):
        ...
        with allure.step("Получаем токен"):
            ...
            allure.attach(
                str(token_item),
                name="Полученный токен",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Создание данных длиннее > 255 символов"):
            item_create = self.wrong_data.too_long_data()
            allure.attach(
                str(item_create),
                name="Полученные данные длинной > 255 символов",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Попытка получения item"):
            response = self.session.post(
                f"{self.base_url}", headers=token_item.headers, json=item_create
            )
            allure.attach(
                str(response),
                name="Ответ на попытку получить item",
                attachment_type=allure.attachment_type.TEXT,
            )
        return response
    ...
    def create_non_title(self):
        ...
        with allure.step("Получение данных с пустым title"):
            ...
            allure.attach(
                str(item_create),
                name="Полученные данные c пустым title",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Попытка получения item"):
            response = self.session.post(
    ...
    def create_empty_token(self):
        ...
        with allure.step("Убираем из header значение 'Authorization'"):
            ...
            allure.attach(
                str(token_item.headers),
                name="Обновленный токен",
        ...
        with allure.step("Получаем данные для создания item"):
            ...
            allure.attach(
                str(item_create),
                name="Полученные данные для создания item",
            )
        with allure.step("Отправляем запрос на создание item без токена"):
            ...
            allure.attach(
                str(response),
                name="Полученный результат создания item без токена",
                attachment_type=allure.attachment_type.TEXT,
            )
        return response
...
class BadScenariosItem:
    ...
    def create_and_get_empty_token(self):
        ...
        with allure.step("Получаем пустой токен"):
            ...
            allure.attach(
                str(empty_token),
                name="результат получения пустого токена",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Отправка GET запроса с пустым токеном"):
            empty_response = self.session.get(
                f"{self.base_url}{id_item}", headers=empty_token.headers
            )
            allure.attach(
                str(empty_response),
                name="результат GET запроса с пустым токеном",
                attachment_type=allure.attachment_type.JSON,
            )
        return response, id_item, empty_response
    ...
    def create_and_update_empty_token(self):
        ...
        with allure.step("Получаем данные для обновления item"):
            ...
            allure.attach(
                str(item_update), name="Результат получения данных для обновления item"
            )
        with allure.step("Отравляем запрос c пустым токен на обновление item"):
            empty_response = self.session.put(
                f"{self.base_url}{id_item}",
                headers=empty_token.headers,
                json=item_update,
            )
            allure.attach(
                str(empty_response),
                name="Результат запроса c пустым токен на обновление item",
                attachment_type=allure.attachment_type.JSON,
            )
        return response, id_item, empty_response
    ...
    def create_and_delete_empty_token(self):
        ...
        with allure.step("Отправка DELETE запроса с пустым токеном"):
            empty_response = self.session.delete(
                f"{self.base_url}{id_item}", headers=empty_token.headers
            )
            allure.attach(
                str(empty_response),
                name="результат DELETE запроса с пустым токеном",
                attachment_type=allure.attachment_type.JSON,
            )
        return response, id_item, empty_response
    ...
    def double_delete_item(self):
        """Сценарий: создание и удаление итем,
        ...
        # 2 Take token for deleting item one more
        with allure.step("Получаем токен для повторного DELETE запроса"):
            token_item = self.token
            allure.attach(
                str(token_item),
                name="токен для удаления item",
                attachment_type=allure.attachment_type.JSON,
            )
        with allure.step("Отправляем ПОВТОРНЫЙ DELETE запрос"):
            double_response = self.session.delete(
                f"{self.base_url}{id_item}", headers=token_item.headers
            )
            allure.attach(
                str(double_response),
                name="Результат повторного удаления item",
                attachment_type=allure.attachment_type.JSON,
            )
        return response, id_item, delete_item, double_response
    ...
    def update_unreal_item(self):
        ...
        with allure.step("Получаем токен для PUT запроса"):
            ...
            allure.attach(
                str(token_item),
                name="Результат получения токена для PUT запроса",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Получаем несуществующий ID"):
            ...
            allure.attach(
                str(unreal_id),
                name="Результат получения несуществующего ID",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Получаем валидные данные для обновления item"):
            ...
            allure.attach(
                str(item_update),
                name="Результат получения данных для обновления item",
                attachment_type=allure.attachment_type.TEXT,
            )
        with allure.step("Отправка PUT запроса с несуществующим ID"):
            response = self.session.put(
                f"{self.base_url}{unreal_id}",
                headers=token_item.headers,
                json=item_update,
            )
            allure.attach(
                str(response),
                name="Результат отправки PUT запроса с несуществующим ID",
                attachment_type=allure.attachment_type.TEXT,
            )
        return response
    ...
    def delete_unreal_item(self):
        ...
        with allure.step("Отправка DELETE запроса с несуществующим ID"):
            response = self.session.delete(
                f"{self.base_url}{unreal_id}", headers=token_item.headers
            )
            allure.attach(
                str(response),
                name="Результат отправки DELETE запроса с несуществующим ID",
                attachment_type=allure.attachment_type.TEXT,
            )
        return response
